"""Integration test: mock closeout safety regression."""
from __future__ import annotations
import sys, pathlib
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent.parent.parent))

from src.runtime_integrations.testnet_mock_closeout.closeout_safety_regression import (
...
def test_no_production_endpoints():
    items = scan_production_endpoints()
    failed = [i for i in items if not i.passed]
    assert len(failed) == 0
