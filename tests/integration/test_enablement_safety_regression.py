"""Integration test: enablement safety regression."""
from __future__ import annotations
import sys, pathlib
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent.parent.parent))

from src.runtime_integrations.testnet_enablement.enablement_safety_regression import (
    run_regression, scan_forbidden_imports, scan_forbidden_statuses,
    scan_real_submit_patterns, scan_locked_gates
)
...
def test_gates_remain_locked():
    items = scan_locked_gates()
    failed = [i for i in items if not i.passed]
    assert len(failed) == 0
