"""Integration test: mock transport safety regression."""
from __future__ import annotations
import sys, pathlib
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent.parent.parent))

from src.runtime_integrations.testnet_mock_transport.mock_transport_safety_regression import (
    run_regression, scan_forbidden_imports, scan_forbidden_statuses,
    scan_real_submit_patterns, scan_env_secrets, scan_suspicious_env_vars
)
...
def test_no_suspicious_env_vars():
    items = scan_suspicious_env_vars()
    failed = [i for i in items if not i.passed]
    assert len(failed) == 0
