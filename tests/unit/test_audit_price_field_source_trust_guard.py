...
_FAKE_ARGV = ["audit_price_field_source_trust"]


def test_import_safe():
    mod = importlib.import_module("scripts.audit_price_field_source_trust")
    assert hasattr(mod, "main")
    assert hasattr(mod, "audit_price_field_source_trust")


def test_no_high_risk_imports():
    source = open(
        "scripts/audit_price_field_source_trust.py", encoding="utf-8"
    ).read()
    for forbidden in [
        "binance_connector",
        "binance_http",
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.audit_price_field_source_trust import main
