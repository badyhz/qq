"""Tests for phase10c trigger recheck script — structure only."""
...
SCRIPT = os.path.join(os.path.dirname(__file__), "..", "..", "scripts", "run_phase10c_trigger_recheck.py")


class TestTriggerRecheckScript:
    ...
    def test_default_symbols(self):
        ...
        spec = importlib.util.spec_from_file_location("recheck", SCRIPT)
    ...
    def test_output_paths(self):
        with open(SCRIPT) as f:
            content = f.read()
        assert "_trigger_recheck.json" in content
        assert "_trigger_recheck.md" in content
        assert "_trigger_recheck.csv" in content
        assert "_trigger_recheck_ledger.jsonl" in content
    ...
    def test_status_order_defined(self):
        ...
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        assert hasattr(mod, "RECHECK_STATUS_ORDER")
        for s in ["TRIGGERED", "WAITING", "INVALIDATED", "SHORT_TRIGGERED",
                   "SHORT_WAITING", "SHORT_INVALIDATED", "DATA_ERROR"]:
            assert s in mod.RECHECK_STATUS_ORDER
