...
_FAKE_ARGV = ["generate_daily_operator_checklist"]


def test_import_safe():
    mod = importlib.import_module("scripts.generate_daily_operator_checklist")
    assert hasattr(mod, "main")
    assert hasattr(mod, "generate_daily_operator_checklist")


def test_no_high_risk_imports():
    source = open(
        "scripts/generate_daily_operator_checklist.py", encoding="utf-8"
    ).read()
    for forbidden in [
        "binance_connector",
        "binance_http",
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.generate_daily_operator_checklist import main
