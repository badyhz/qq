...
_FAKE_ARGV = ["evaluate_missing_klines_recovery"]


def test_import_safe():
    mod = importlib.import_module("scripts.evaluate_missing_klines_recovery")
    assert hasattr(mod, "main")
    assert hasattr(mod, "evaluate_missing_klines_recovery")


def test_no_high_risk_imports():
    source = open(
        "scripts/evaluate_missing_klines_recovery.py", encoding="utf-8"
    ).read()
    for forbidden in [
        "binance_connector",
        "binance_http",
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.evaluate_missing_klines_recovery import main
