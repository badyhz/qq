...
_FAKE_ARGV = ["analyze_readiness_blocker_attribution"]

def test_import_safe():
    mod = importlib.import_module("scripts.analyze_readiness_blocker_attribution")
    assert hasattr(mod, "main")
    assert hasattr(mod, "analyze_readiness_blocker_attribution")

def test_no_high_risk_imports():
    source = open("scripts/analyze_readiness_blocker_attribution.py", encoding="utf-8").read()
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.analyze_readiness_blocker_attribution import main
