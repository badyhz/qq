...
_FAKE_ARGV = ["generate_sample_collection_eod_report"]


def test_import_safe():
    mod = importlib.import_module("scripts.generate_sample_collection_eod_report")
    assert hasattr(mod, "main")
    assert hasattr(mod, "generate_sample_collection_eod_report")


def test_no_high_risk_imports():
    source = open(
        "scripts/generate_sample_collection_eod_report.py", encoding="utf-8"
    ).read()
    for forbidden in [
        "binance_connector",
        "binance_http",
...
def test_missing_mode_blocked(monkeypatch):
    ...
    from scripts.generate_sample_collection_eod_report import main
