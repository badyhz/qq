...
_FAKE_ARGV = ["generate_runner_dry_run_report"]


def test_import_safe():
    mod = importlib.import_module("scripts.generate_runner_dry_run_report")
    assert hasattr(mod, "main")
    assert hasattr(mod, "generate_runner_dry_run_report")


def test_no_high_risk_imports():
    source = open(
        "scripts/generate_runner_dry_run_report.py", encoding="utf-8"
    ).read()
    for forbidden in [
        "binance_connector",
        "binance_http",
...
def test_missing_mode_blocked(monkeypatch):
    ...
    from scripts.generate_runner_dry_run_report import main
