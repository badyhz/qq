...
_FAKE_ARGV = ["validate_real_ohlcv_gap_candidates"]


def test_import_safe():
    mod = importlib.import_module("scripts.validate_real_ohlcv_gap_candidates")
    assert hasattr(mod, "main")
    assert hasattr(mod, "validate_real_ohlcv_gap_candidates")


def test_no_high_risk_imports():
    source = open(
        "scripts/validate_real_ohlcv_gap_candidates.py", encoding="utf-8"
    ).read()
    for forbidden in [
        "binance_connector",
        "binance_http",
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.validate_real_ohlcv_gap_candidates import main
