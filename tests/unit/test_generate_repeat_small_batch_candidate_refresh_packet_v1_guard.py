...
_FAKE_ARGV = ["generate_repeat_small_batch_candidate_refresh_packet_v1"]
_DRY_RUN_ARGV = [
    "generate_repeat_small_batch_candidate_refresh_packet_v1",
    "--repeat-eligibility-json", "/dev/null",
    "--new-candidates-json", "/dev/null",
]


def test_import_safe():
    mod = importlib.import_module("scripts.generate_repeat_small_batch_candidate_refresh_packet_v1")
    assert hasattr(mod, "main")
    assert hasattr(mod, "generate_refresh")


def test_no_high_risk_imports():
    source = open(
        "scripts/generate_repeat_small_batch_candidate_refresh_packet_v1.py", encoding="utf-8"
    ).read()
    for forbidden in [
        "binance_connector",
        "binance_http",
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.generate_repeat_small_batch_candidate_refresh_packet_v1 import main
