...
_FAKE_ARGV = ["analyze_trade_lifecycle_performance"]


def test_import_safe():
    mod = importlib.import_module("scripts.analyze_trade_lifecycle_performance")
    assert hasattr(mod, "main")
    assert hasattr(mod, "analyze_trade_lifecycle_performance")


def test_no_high_risk_imports():
    source = open(
        "scripts/analyze_trade_lifecycle_performance.py", encoding="utf-8"
    ).read()
    for forbidden in [
        "binance_connector",
        "binance_http",
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.analyze_trade_lifecycle_performance import main
