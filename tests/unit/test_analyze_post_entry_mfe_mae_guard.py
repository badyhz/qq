...
_FAKE_ARGV = ["analyze_post_entry_mfe_mae"]


def test_import_safe():
    mod = importlib.import_module("scripts.analyze_post_entry_mfe_mae")
    assert hasattr(mod, "main")
    assert hasattr(mod, "analyze_post_entry_mfe_mae")


def test_no_high_risk_imports():
    source = open(
        "scripts/analyze_post_entry_mfe_mae.py", encoding="utf-8"
    ).read()
    for forbidden in [
        "binance_connector",
        "binance_http",
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.analyze_post_entry_mfe_mae import main
