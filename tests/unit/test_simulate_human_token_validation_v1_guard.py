...
_FAKE_ARGV = ["simulate_human_token_validation_v1", "--human-token-packet-json", "/dev/null"]


def test_import_safe():
    mod = importlib.import_module("scripts.simulate_human_token_validation_v1")
    assert hasattr(mod, "main")
    assert hasattr(mod, "load_json")
    assert hasattr(mod, "write_json")
    assert hasattr(mod, "generate_validation")


def test_no_high_risk_imports():
    source = open(
        "scripts/simulate_human_token_validation_v1.py", encoding="utf-8"
    ).read()
    for forbidden in [
        "binance_connector",
        "binance_http",
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.simulate_human_token_validation_v1 import main
