...
_FAKE_ARGV = ["verify_human_copy_paste_dry_run_command_v1"]
_DRY_RUN_ARGV = [
    "verify_human_copy_paste_dry_run_command_v1",
    "--dry-run-readiness-json", "/dev/null",
]


def test_import_safe():
    mod = importlib.import_module("scripts.verify_human_copy_paste_dry_run_command_v1")
    assert hasattr(mod, "main")
    assert hasattr(mod, "verify_command")


def test_no_high_risk_imports():
    source = open(
        "scripts/verify_human_copy_paste_dry_run_command_v1.py", encoding="utf-8"
    ).read()
    for forbidden in [
        "binance_connector",
        "binance_http",
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.verify_human_copy_paste_dry_run_command_v1 import main
