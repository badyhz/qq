...
def test_import_safe():
    mod = importlib.import_module("scripts.generate_single_human_gated_execution_command_preview_packet_v1")
    assert hasattr(mod, "main")
    assert hasattr(mod, "generate_preview")

def test_no_high_risk_imports():
    source = open("scripts/generate_single_human_gated_execution_command_preview_packet_v1.py", encoding="utf-8").read()
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.generate_single_human_gated_execution_command_preview_packet_v1 import main
