...
_FAKE_ARGV = ["audit_real_ohlcv_source_schema"]


def test_import_safe():
    mod = importlib.import_module("scripts.audit_real_ohlcv_source_schema")
    assert hasattr(mod, "main")
    assert hasattr(mod, "audit_real_ohlcv_source_schema")


def test_no_high_risk_imports():
    source = open(
        "scripts/audit_real_ohlcv_source_schema.py", encoding="utf-8"
    ).read()
    for forbidden in [
        "binance_connector",
        "binance_http",
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.audit_real_ohlcv_source_schema import main
