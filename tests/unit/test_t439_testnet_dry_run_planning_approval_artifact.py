...
from scripts.generate_testnet_dry_run_planning_approval_artifact_v1 import (
...
def test_valid_t438_gives_pass(tmp_path):
    result = generate_approval_artifact(valid_t438())

    assert result["ok"] is True
    assert result["task"] == "T439"
    assert result["approval_status"] == "TESTNET_DRY_RUN_PLANNING_APPROVED"
    assert result["approval_scope"] == "APPROVE_TESTNET_DRY_RUN_READINESS_REVIEW_ONLY"
    assert result["final_decision"] == "READY_FOR_TESTNET_DRY_RUN_PLANNING_FINAL_GATE"


def test_blocked_t438_gives_blocked(tmp_path):
    bad_t438 = valid_t438()
    bad_t438["ok"] = False
    bad_t438["final_decision"] = "CONTINUE_TESTNET_DRY_RUN_PLANNING_REVIEW"

    result = generate_approval_artifact(bad_t438)
...
def test_approval_scope_is_readiness_review_only(tmp_path):
    result = generate_approval_artifact(valid_t438())
...
def test_missing_t438_gives_blocked(tmp_path):
...
def test_cli_smoke(tmp_path):
    t438_path = tmp_path / "t438.json"

    with open(t438_path, "w") as f:
        json.dump(valid_t438(), f)

    proc = subprocess.Popen(
        [
            sys.executable,
            str(Path(__file__).parent.parent.parent / "scripts" / "generate_testnet_dry_run_planning_approval_artifact_v1.py"),
            "--risk-review-report", str(t438_path),
            "--json"
        ],
