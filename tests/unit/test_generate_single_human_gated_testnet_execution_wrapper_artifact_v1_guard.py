...
_FAKE_ARGV = ["generate_single_human_gated_testnet_execution_wrapper_artifact_v1"]
_DRY_RUN_ARGV = [
    "generate_single_human_gated_testnet_execution_wrapper_artifact_v1",
    "--wrapper-phase-json", "/dev/null",
    "--final-safety-gate-json", "/dev/null",
    "--dry-run-plan-json", "/dev/null",
    "--single-submit-packet-json", "/dev/null",
    "--human-token-packet-json", "/dev/null",
]


def test_import_safe():
    mod = importlib.import_module("scripts.generate_single_human_gated_testnet_execution_wrapper_artifact_v1")
    assert hasattr(mod, "main")
    assert hasattr(mod, "generate_artifact")


def test_no_high_risk_imports():
    source = open(
        "scripts/generate_single_human_gated_testnet_execution_wrapper_artifact_v1.py", encoding="utf-8"
    ).read()
    for forbidden in [
        "binance_connector",
        "binance_http",
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.generate_single_human_gated_testnet_execution_wrapper_artifact_v1 import main
