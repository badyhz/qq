...
_FAKE_ARGV = ["generate_phase_control_report_v2"]


def test_import_safe():
    mod = importlib.import_module("scripts.generate_phase_control_report_v2")
    assert hasattr(mod, "main")
    assert hasattr(mod, "generate_phase_control_report_v2")


def test_no_high_risk_imports():
    source = open(
        "scripts/generate_phase_control_report_v2.py", encoding="utf-8"
    ).read()
    for forbidden in [
        "binance_connector",
        "binance_http",
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.generate_phase_control_report_v2 import main
