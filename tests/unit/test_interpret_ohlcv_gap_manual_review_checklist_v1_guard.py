...
_FAKE_ARGV = ["interpret_ohlcv_gap_manual_review_checklist_v1"]


def test_import_safe():
    mod = importlib.import_module("scripts.interpret_ohlcv_gap_manual_review_checklist_v1")
    assert hasattr(mod, "main")
    assert hasattr(mod, "interpret_ohlcv_gap_manual_review_checklist_v1")


def test_no_high_risk_imports():
    source = open(
        "scripts/interpret_ohlcv_gap_manual_review_checklist_v1.py", encoding="utf-8"
    ).read()
    for forbidden in [
        "binance_connector",
        "binance_http",
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.interpret_ohlcv_gap_manual_review_checklist_v1 import main
