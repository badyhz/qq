...
_FAKE_ARGV = ["evaluate_strategy_promotion_rules"]

def test_import_safe():
    mod = importlib.import_module("scripts.evaluate_strategy_promotion_rules")
    assert hasattr(mod, "main")
    assert hasattr(mod, "evaluate_strategy_promotion_rules")

def test_no_high_risk_imports():
    source = open("scripts/evaluate_strategy_promotion_rules.py", encoding="utf-8").read()
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.evaluate_strategy_promotion_rules import main
