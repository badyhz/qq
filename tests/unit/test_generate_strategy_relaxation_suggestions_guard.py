...
_FAKE_ARGV = ["generate_strategy_relaxation_suggestions"]


def test_import_safe():
    mod = importlib.import_module("scripts.generate_strategy_relaxation_suggestions")
    assert hasattr(mod, "main")
    assert hasattr(mod, "generate_strategy_relaxation_suggestions")


def test_no_high_risk_imports():
    source = open(
        "scripts/generate_strategy_relaxation_suggestions.py", encoding="utf-8"
    ).read()
    for forbidden in [
        "binance_connector",
        "binance_http",
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.generate_strategy_relaxation_suggestions import main
