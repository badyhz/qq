...
_FAKE_ARGV = ["map_readiness_blockers_to_actions"]

def test_import_safe():
    mod = importlib.import_module("scripts.map_readiness_blockers_to_actions")
    assert hasattr(mod, "main")
    assert hasattr(mod, "map_readiness_blockers_to_actions")

def test_no_high_risk_imports():
    source = open("scripts/map_readiness_blockers_to_actions.py", encoding="utf-8").read()
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.map_readiness_blockers_to_actions import main
