"""Tests for phase10c daily readonly shadow script — structure only."""
...
SCRIPT = os.path.join(os.path.dirname(__file__), "..", "..", "scripts", "run_phase10c_daily_readonly_shadow.py")


class TestPhase10C2Script:
    ...
    def test_date_option(self):
        ...
        assert "YYYY-MM-DD" in content or "%Y-%m-%d" in content

    def test_daily_output_paths_date_scoped(self):
        with open(SCRIPT) as f:
            content = f.read()
        assert "{date_str}_shadow_ledger.jsonl" in content
        assert "{date_str}_shadow_summary.json" in content
        assert "{date_str}_shadow_report.md" in content

    def test_cumulative_output_paths(self):
        with open(SCRIPT) as f:
            content = f.read()
        assert "cumulative_shadow_ledger.jsonl" in content
        assert "cumulative_shadow_summary.json" in content
        assert "cumulative_shadow_report.md" in content
    ...
    def test_safety_flags_present(self):
        ...
        spec = importlib.util.spec_from_file_location("daily10c2", SCRIPT)
