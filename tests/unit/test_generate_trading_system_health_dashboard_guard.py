...
_FAKE_ARGV = ["generate_trading_system_health_dashboard"]


def test_import_safe():
    mod = importlib.import_module("scripts.generate_trading_system_health_dashboard")
    assert hasattr(mod, "main")
    assert hasattr(mod, "generate_trading_system_health_dashboard")


def test_no_high_risk_imports():
    source = open(
        "scripts/generate_trading_system_health_dashboard.py", encoding="utf-8"
    ).read()
    for forbidden in [
        "binance_connector",
        "binance_http",
...
def test_missing_mode_blocked(monkeypatch):
    ...
    from scripts.generate_trading_system_health_dashboard import main
