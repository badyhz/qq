...
def test_import_safe():
    mod = importlib.import_module("scripts.generate_human_gated_execution_wrapper_phase_control_report_v1")
    assert hasattr(mod, "main")
    assert hasattr(mod, "generate_phase_report")

def test_no_high_risk_imports():
    source = open("scripts/generate_human_gated_execution_wrapper_phase_control_report_v1.py", encoding="utf-8").read()
...
def test_default_dry_run_allowed(monkeypatch):
    ...
    from scripts.generate_human_gated_execution_wrapper_phase_control_report_v1 import main
