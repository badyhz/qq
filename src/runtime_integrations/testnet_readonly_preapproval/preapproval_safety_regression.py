"""Preapproval no-network no-submit safety regression scanner."""
from __future__ import annotations
import json, pathlib, re
import subprocess
from dataclasses import dataclass

PREAPPROVAL_DIR = pathlib.Path(__file__).parent
...
def scan_forbidden_imports() -> list[RegressionItem]:
    ...
    for py_file in sorted(PREAPPROVAL_DIR.glob("*.py")):
...
def scan_high_risk_legacy() -> list[RegressionItem]:
    items: list[RegressionItem] = []
    project_root = PREAPPROVAL_DIR.parent.parent.parent
...
def scan_env_secrets() -> list[RegressionItem]:
    ...
    for py_file in sorted(PREAPPROVAL_DIR.glob("*.py")):
        if py_file.resolve() == self_path:
            continue
        content = py_file.read_text(encoding="utf-8")
        if re.search(r"os\.environ|getenv|open\(.*\.env|open\(.*key|open\(.*secret", content):
            items.append(RegressionItem(f"env_secret_{py_file.stem}", False, f"Environment/.env secret access found in {py_file.name}"))
    if not items:
        items.append(RegressionItem("env_secrets_all", True, "No environment/.env secret access found"))
    return items
...
def render_report(items: list[RegressionItem]) -> str:
    lines = ["# Preapproval Safety Regression", "",
        "**Status: READONLY_PREAPPROVAL_NO_NETWORK_NO_SUBMIT_SAFETY_PASS**",
    ...
    lines.extend(["", "## Conclusion", "", "READONLY_PREAPPROVAL_NO_NETWORK_NO_SUBMIT_SAFETY_PASS", "TESTNET_SUBMIT_NOT_ALLOWED", ""])
    return "\n".join(lines)
