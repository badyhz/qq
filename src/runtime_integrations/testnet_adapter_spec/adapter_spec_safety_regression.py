"""Adapter spec safety regression scanner."""
...
def scan_env_secrets() -> list[RegressionItem]:
    ...
    for py_file in sorted(SPEC_DIR.glob("*.py")):
        ...
        if re.search(r"os\.environ|getenv|open\(.*key|open\(.*secret", content):
            items.append(RegressionItem(
                f"env_secret_{py_file.stem}",
                False,
                f"Environment secret access found in {py_file.name}"
            ))
    if not items:
...
def render_report(items: list[RegressionItem]) -> str:
    lines = ["# Adapter Spec Safety Regression", "",
        "**Status: NO_SUBMIT_SAFETY_PRESERVED**",
    ...
    lines.extend(["", "## Conclusion", "", "NO_SUBMIT_SAFETY_PRESERVED", "TESTNET_SUBMIT_NOT_ALLOWED", ""])
    return "\n".join(lines)
