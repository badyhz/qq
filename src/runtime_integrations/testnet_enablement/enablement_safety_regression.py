"""Enablement safety regression scanner."""
from __future__ import annotations
import json, pathlib, re
import subprocess
from dataclasses import dataclass

ENABLEMENT_DIR = pathlib.Path(__file__).parent
...
def scan_forbidden_imports() -> list[RegressionItem]:
    ...
    for py_file in sorted(ENABLEMENT_DIR.glob("*.py")):
        ...
        for imp in FORBIDDEN_IMPORTS:
            ...
            if re.search(pattern, content, re.MULTILINE):
                items.append(RegressionItem(
                    f"forbidden_import_{imp}_{py_file.stem}",
                    False,
                    f"Forbidden import '{imp}' found in {py_file.name}"
                ))
    if not items:
    ...
    for py_file in sorted(ENABLEMENT_DIR.glob("*.py")):
        ...
        for status in FORBIDDEN_STATUSES:
            if status in content:
                items.append(RegressionItem(
                    f"forbidden_status_{status}_{py_file.stem}",
                    False,
                    f"Forbidden status '{status}' found in {py_file.name}"
                ))
    if not items:
...
def scan_real_submit_patterns() -> list[RegressionItem]:
    ...
    for py_file in sorted(ENABLEMENT_DIR.glob("*.py")):
        ...
        if re.search(r"real_submit\s*[:=]\s*True", content):
            items.append(RegressionItem(
                f"real_submit_true_{py_file.stem}",
                False,
                f"real_submit=True found in {py_file.name}"
            ))
    if not items:
...
def scan_high_risk_legacy() -> list[RegressionItem]:
    items: list[RegressionItem] = []
    project_root = ENABLEMENT_DIR.parent.parent.parent
    for legacy_path in HIGH_RISK_LEGACY:
        ...
        if full_path.exists():
            ...
            if not tracked:
                items.append(RegressionItem(
                    f"high_risk_legacy_untracked_{legacy_path.replace('/', '_')}",
                    True,
                    f"Known isolated high-risk legacy file remains untracked: {legacy_path}"
                ))
                continue
            items.append(RegressionItem(
                f"high_risk_legacy_{legacy_path.replace('/', '_')}",
                False,
                f"High-risk legacy file is tracked: {legacy_path}"
            ))
    if not items:
        items.append(RegressionItem("high_risk_legacy_all", True, "No high-risk legacy files found"))
    return items

def scan_locked_gates() -> list[RegressionItem]:
    items: list[RegressionItem] = []
    self_path = pathlib.Path(__file__).resolve()
    for py_file in sorted(ENABLEMENT_DIR.glob("*.py")):
        if py_file.resolve() == self_path:
            continue
        content = py_file.read_text(encoding="utf-8")
        if "submit_allowed" in content and "submit_allowed=True" in content.lower().replace(" ", ""):
            items.append(RegressionItem(
                f"submit_unlocked_{py_file.stem}",
                False,
                f"submit_allowed=True found in {py_file.name}"
            ))
    if not items:
        items.append(RegressionItem("locked_gates_all", True, "All gates remain locked"))
    return items
...
def run_regression() -> list[RegressionItem]:
    ...
    results.extend(scan_locked_gates())
    return results
...
def render_report(items: list[RegressionItem]) -> str:
    lines = ["# Enablement Safety Regression", "", "**Status: SAFETY_REGRESSION_DOCUMENTED**", "**Submit: TESTNET_SUBMIT_NOT_ALLOWED**", ""]
    ...
    lines.extend(["", "## Conclusion", "", "ENABLEMENT_SAFETY_REGRESSION_DOCUMENTED", "TESTNET_SUBMIT_NOT_ALLOWED", ""])
    return "\n".join(lines)
