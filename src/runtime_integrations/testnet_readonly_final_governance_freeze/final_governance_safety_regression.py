"""Final governance no-network no-submit safety regression scanner."""
from __future__ import annotations
import json, pathlib, re, subprocess
from dataclasses import dataclass

GOVERNANCE_DIR = pathlib.Path(__file__).parent
...
def scan_forbidden_imports() -> list[RegressionItem]:
    ...
    for py_file in sorted(GOVERNANCE_DIR.glob("*.py")):
...
def scan_high_risk_legacy() -> list[RegressionItem]:
    items: list[RegressionItem] = []
    project_root = GOVERNANCE_DIR.parent.parent.parent
...
def scan_env_secrets() -> list[RegressionItem]:
    ...
    for py_file in sorted(GOVERNANCE_DIR.glob("*.py")):
        ...
        if re.search(r"os\.environ|getenv|open\(.*\.env|open\(.*key|open\(.*secret", content):
            items.append(RegressionItem(f"env_secret_{py_file.stem}", False, f"Environment/dotenv secret access found in {py_file.name}"))
    if not items:
        items.append(RegressionItem("env_secrets_all", True, "No environment/dotenv secret access found"))
    return items
...
def render_report(items: list[RegressionItem]) -> str:
    lines = ["# Final Governance Safety Regression", "",
        "**Status: READONLY_FINAL_GOVERNANCE_NO_NETWORK_NO_SUBMIT_SAFETY_PASS**",
