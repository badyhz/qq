"""Read-only discovery safety regression scanner."""
from __future__ import annotations
import json, pathlib, re
import subprocess
from dataclasses import dataclass

DISCOVERY_DIR = pathlib.Path(__file__).parent
...
def scan_high_risk_legacy() -> list[RegressionItem]:
    items: list[RegressionItem] = []
    project_root = DISCOVERY_DIR.parent.parent.parent
...
def render_report(items: list[RegressionItem]) -> str:
    lines = ["# Read-Only Discovery Safety Regression", "",
        "**Status: READONLY_DISCOVERY_NO_NETWORK_NO_SUBMIT_SAFETY_PASS**",
    ...
    lines.extend(["", "## Conclusion", "", "READONLY_DISCOVERY_NO_NETWORK_NO_SUBMIT_SAFETY_PASS", "TESTNET_SUBMIT_NOT_ALLOWED", ""])
    return "\n".join(lines)
