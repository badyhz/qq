"""Final approval no-network no-submit safety regression scanner."""
from __future__ import annotations
import json, pathlib, re, subprocess
from dataclasses import dataclass

FINAL_APPROVAL_DIR = pathlib.Path(__file__).parent
...
def scan_forbidden_imports() -> list[RegressionItem]:
    ...
    for py_file in sorted(FINAL_APPROVAL_DIR.glob("*.py")):
...
def scan_high_risk_legacy() -> list[RegressionItem]:
    items: list[RegressionItem] = []
    project_root = FINAL_APPROVAL_DIR.parent.parent.parent
...
def render_report(items: list[RegressionItem]) -> str:
    lines = ["# Final Approval Safety Regression", "",
        "**Status: READONLY_FINAL_APPROVAL_NO_NETWORK_NO_SUBMIT_SAFETY_PASS**",
