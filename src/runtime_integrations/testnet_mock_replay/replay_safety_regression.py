"""Replay no-submit safety regression scanner."""
...
def scan_forbidden_imports() -> list[RegressionItem]:
    ...
    for py_file in sorted(REPLAY_DIR.glob("*.py")):
...
def scan_high_risk_legacy() -> list[RegressionItem]:
    items: list[RegressionItem] = []
    project_root = REPLAY_DIR.parent.parent.parent
...
def render_report(items: list[RegressionItem]) -> str:
    lines = ["# Replay No-Submit Safety Regression", "",
        "**Status: MOCK_REPLAY_NO_SUBMIT_SAFETY_REGRESSION_PASS**",
    ...
    lines.extend(["", "## Conclusion", "", "MOCK_REPLAY_NO_SUBMIT_SAFETY_REGRESSION_PASS", "TESTNET_SUBMIT_NOT_ALLOWED", ""])
    return "\n".join(lines)
