"""Closeout no-submit safety regression scanner."""
from __future__ import annotations
import json, pathlib, re
import subprocess
from dataclasses import dataclass

CLOSEOUT_DIR = pathlib.Path(__file__).parent
...
def scan_forbidden_imports() -> list[RegressionItem]:
    ...
    for py_file in sorted(CLOSEOUT_DIR.glob("*.py")):
...
def scan_high_risk_legacy() -> list[RegressionItem]:
    items: list[RegressionItem] = []
    project_root = CLOSEOUT_DIR.parent.parent.parent
...
def scan_production_endpoints() -> list[RegressionItem]:
    items: list[RegressionItem] = []
    self_path = pathlib.Path(__file__).resolve()
    for py_file in sorted(CLOSEOUT_DIR.glob("*.py")):
        if py_file.resolve() == self_path:
        ...
        if re.search(r"https?://api\.binance\.com(?!/test)", content):
            items.append(RegressionItem(f"prod_endpoint_{py_file.stem}", False, f"Production endpoint found in {py_file.name}"))
    if not items:
        items.append(RegressionItem("prod_endpoints_all", True, "No production endpoints found"))
    return items
...
def run_regression() -> list[RegressionItem]:
    ...
    results.extend(scan_production_endpoints())
    return results
...
def render_report(items: list[RegressionItem]) -> str:
    lines = ["# Closeout No-Submit Safety Regression", "",
        "**Status: MOCK_CLOSEOUT_NO_SUBMIT_SAFETY_REGRESSION_PASS**",
    ...
    lines.extend(["", "## Conclusion", "", "MOCK_CLOSEOUT_NO_SUBMIT_SAFETY_REGRESSION_PASS", "TESTNET_SUBMIT_NOT_ALLOWED", ""])
    return "\n".join(lines)
