"""Release gate no-network no-submit safety regression scanner."""
from __future__ import annotations
import json, pathlib, re, subprocess
from dataclasses import dataclass

RELEASE_GATE_DIR = pathlib.Path(__file__).parent
...
def scan_forbidden_imports() -> list[RegressionItem]:
    ...
    for py_file in sorted(RELEASE_GATE_DIR.glob("*.py")):
...
def scan_high_risk_legacy() -> list[RegressionItem]:
    items: list[RegressionItem] = []
    project_root = RELEASE_GATE_DIR.parent.parent.parent
...
def render_report(items: list[RegressionItem]) -> str:
    lines = ["# Release Gate Safety Regression", "",
        "**Status: READONLY_RELEASE_GATE_NO_NETWORK_NO_SUBMIT_SAFETY_PASS**",
