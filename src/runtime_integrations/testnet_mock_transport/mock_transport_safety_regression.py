"""Mock transport no-submit safety regression scanner."""
...
SUSPICIOUS_ENV_VARS = ("API_KEY", "API_SECRET", "BINANCE_API_KEY", "BINANCE_SECRET", "EXCHANGE_KEY", "EXCHANGE_SECRET")
...
def scan_forbidden_imports() -> list[RegressionItem]:
    ...
    for py_file in sorted(MOCK_DIR.glob("*.py")):
...
def scan_high_risk_legacy() -> list[RegressionItem]:
    items: list[RegressionItem] = []
    project_root = MOCK_DIR.parent.parent.parent
...
def scan_suspicious_env_vars() -> list[RegressionItem]:
    items: list[RegressionItem] = []
    self_path = pathlib.Path(__file__).resolve()
    for py_file in sorted(MOCK_DIR.glob("*.py")):
        if py_file.resolve() == self_path:
        ...
        for env_var in SUSPICIOUS_ENV_VARS:
            if env_var in content and "SUSPICIOUS_ENV_VARS" not in content and f'"{env_var}"' not in content:
                items.append(RegressionItem(f"suspicious_env_{env_var}_{py_file.stem}", False, f"Suspicious env var '{env_var}' referenced in {py_file.name}"))
    if not items:
        items.append(RegressionItem("suspicious_env_all", True, "No suspicious env var references found"))
    return items
...
def run_regression() -> list[RegressionItem]:
    ...
    results.extend(scan_suspicious_env_vars())
    return results
...
def render_report(items: list[RegressionItem]) -> str:
    lines = ["# Mock Transport No-Submit Safety Regression", "",
        "**Status: MOCK_TRANSPORT_NO_SUBMIT_SAFETY_REGRESSION_PASS**",
    ...
    lines.extend(["", "## Conclusion", "", "MOCK_TRANSPORT_NO_SUBMIT_SAFETY_REGRESSION_PASS", "TESTNET_SUBMIT_NOT_ALLOWED", ""])
    return "\n".join(lines)
