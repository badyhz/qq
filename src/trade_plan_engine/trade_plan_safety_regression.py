"""Trade plan engine safety regression scanner."""
from __future__ import annotations
import json, pathlib, re, uuid
from dataclasses import dataclass
from datetime import datetime, timezone

from src.trade_plan_engine.models import new_id
...
def _scan_file(path: pathlib.Path) -> list[RegressionCheck]:
    ...
    for pat in FORBIDDEN_PATTERNS:
        if pat in content:
            # Allow safe aliases in string literals
            if pat in ("api_key", "api_secret") and f'"{pat}"' not in content and f"'{pat}'" not in content:
                continue
...
def run_safety_regression() -> SafetyRegressionReport:
    checks: list[RegressionCheck] = []
    py_files = sorted(ENGINE_ROOT.glob("*.py"))
    for f in py_files:
        if f.name == "trade_plan_safety_regression.py":
            continue
    ...
    verdict = "TRADE_PLAN_ENGINE_NO_SUBMIT_SAFETY_PASS" if flagged == 0 else "TRADE_PLAN_ENGINE_SAFETY_FLAGGED"
    return SafetyRegressionReport(
        report_id=new_id("TSR"),
...
def render_report(report: SafetyRegressionReport) -> str:
    lines = ["# Trade Plan Engine Safety Regression", "",
