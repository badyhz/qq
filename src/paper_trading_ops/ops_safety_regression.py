"""Ops safety regression scanner."""
from __future__ import annotations
import json, pathlib, re
from dataclasses import dataclass
from datetime import datetime, timezone
from src.paper_trading_ops.models import new_id
...
FORBIDDEN_PATTERNS = (
    ...
    "FEISHU_SECRET", "load_dotenv", "systemctl enable", "systemctl start",
    "crontab -e",
)
REAL_ENDPOINTS = ("api.binance.com", "fapi.binance.com", "testnet.binance.vision")
OPS_ROOT = pathlib.Path(__file__).resolve().parent
...
def _scan_file(path: pathlib.Path) -> list[RegressionCheck]:
    ...
    try:
        ...
        return checks
    rel = str(path.relative_to(OPS_ROOT))
    ...
    for pat in FORBIDDEN_PATTERNS:
        if pat in content:
            checks.append(RegressionCheck(f"SR_{rel}_{pat.replace(' ', '_')}", rel, "FLAGGED",
...
def run_safety_regression() -> SafetyRegressionReport:
    checks: list[RegressionCheck] = []
    py_files = sorted(OPS_ROOT.glob("*.py"))
    for f in py_files:
        if f.name == "ops_safety_regression.py":
            continue
    ...
    verdict = "PAPER_TRADING_OPS_NO_SUBMIT_SAFETY_PASS" if flagged == 0 else "PAPER_TRADING_OPS_SAFETY_FLAGGED"
    return SafetyRegressionReport(
        report_id=new_id("OSR"), created_at=datetime.now(timezone.utc).isoformat(),
