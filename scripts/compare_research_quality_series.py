#!/usr/bin/env python3
"""Compare research quality series — direct quality gate directory comparison.

Usage:
    python3 scripts/compare_research_quality_series.py \
        --bundle baseline=/tmp/multi_strategy_research_quality_gate \
        --bundle candidate=/tmp/multi_strategy_research_quality_gate_rerun \
        --output-dir /tmp/research_quality_series_compare \
        --strict \
        --release-hold HOLD
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare research quality series")
    parser.add_argument(
        "--bundle", action="append", required=True,
        help="label=/path/to/quality_gate_dir (repeat for multiple bundles)",
    )
