...
def validate_enriched_real_shadow_records(
    build_result: dict[str, Any] | None = None,
    output_dir: str = "reports/enriched_real_shadow_validation",
) -> dict[str, Any]:
    ...
    if build_result:
        ...
        for record in input_records:
            ...
            # Check required fields including OHLCV
            required_fields = ["record_id", "source_type", "symbol", "timeframe", "setup", "timestamp", "open", "high", "low", "close", "volume"]
            has_all_fields = True
            ...
            # Check OHLCV complete and numeric
            ohlcv_complete = has_all_fields
            if has_all_fields:
                try:
                    float(record["open"])
                    float(record["high"])
    ...
    quality_passed = (
        ...
        ohlcv_complete_records == valid_records
    )
    ...
    report = {
        "task_id": "T399",
        "phase": "ENRICHED_REAL_SHADOW_RECORD_VALIDATION",
    ...
    report_json = out_dir / "enriched_real_shadow_validation.json"
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate enriched real shadow records")
    parser.add_argument("--output-dir", default="reports/enriched_real_shadow_validation")
...
def main() -> None:
    ...
    result = validate_enriched_real_shadow_records(
