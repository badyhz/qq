...
def generate_testnet_dry_run_readiness_v4(
    *,
    ...
    if _to_bool(conv_v3.get("remediation_effective")):
        remediation_effective = True
    if _to_bool(third_loop.get("remediation_effective")):
        remediation_effective = True

    sample_gap_closed = False
    gap_latest = _to_int(conv_v3.get("gap_latest", 0))
    if gap_latest <= 0:
        gap_latest = _to_int(third_loop.get("sample_gap_after", 0))
    if gap_latest == 0:
        sample_gap_closed = True
    ...
    if remediation_effective and sample_gap_closed and convergence_confirmed:
        multi_round_confirmation_ok = True
    ...
    gates = {
        ...
        "history_dedup_ok": history_dedup_ok,
        "shadow_only_integrity_ok": shadow_only_integrity_ok,
        "multi_round_confirmation_ok": multi_round_confirmation_ok,
    }
    ...
    if gates["shadow_only_integrity_ok"]:
        readiness_score += 5.0
    if gates["multi_round_confirmation_ok"]:
        readiness_score += 15.0

    readiness_trend = "UNKNOWN"
    prev_readiness = _to_float_nan(readiness_v3.get("readiness_score"))
    ...
    if not gates["multi_round_confirmation_ok"]:
        blocked_reasons.append("multi_round_confirmation_not_achieved")
    if missing_inputs:
    ...
    report_json = out_dir / "testnet_dry_run_readiness_v4_report.json"
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate testnet dry-run readiness v4 report")
    ...
    parser.add_argument("--output-dir", default="reports/testnet_dry_run_readiness_v4")
...
def main() -> None:
    ...
    result = generate_testnet_dry_run_readiness_v4(
        ...
        output_dir=str(args.output_dir or "reports/testnet_dry_run_readiness_v4"),
    )
    if bool(args.json):
