"""Suite runner: testnet read-only final governance freeze T305001-T320000."""
from __future__ import annotations
import importlib, json, pathlib, sys, time

ROOT = pathlib.Path(__file__).resolve().parent.parent
sys.path.insert(0, str(ROOT))

OUT_DIR = ROOT / "data" / "runtime" / "testnet_readonly_final_governance_freeze"
REPORT_DIR = ROOT / "reports" / "testnet_readonly_final_governance_freeze"

STEPS = [
    ("step_01_final_governance_freeze", "scripts.run_readonly_final_governance_freeze"),
    ("step_02_operator_handoff_packet", "scripts.run_operator_handoff_packet"),
    ("step_03_no_submit_release_archive", "scripts.run_no_submit_release_archive"),
    ("step_04_safety_regression", "scripts.run_final_governance_safety_regression"),
]


def run_step(step_id: str, mod_path: str) -> dict:
    try:
        mod = importlib.import_module(mod_path)
        rc = mod.main()
        return {"step_id": step_id, "status": "PASS" if rc == 0 else "FAIL", "return_code": rc}
    except Exception as exc:
        return {"step_id": step_id, "status": "FAIL", "error": str(exc)}


def run_previous_suites() -> list[dict]:
    results = []
    suites = [
        ("readonly_discovery_suite", "scripts.run_testnet_readonly_discovery_suite"),
        ("readonly_preapproval_suite", "scripts.run_testnet_readonly_preapproval_suite"),
        ("readonly_release_gate_suite", "scripts.run_testnet_readonly_release_gate_suite"),
        ("readonly_final_approval_simulator_suite", "scripts.run_testnet_readonly_final_approval_simulator_suite"),
        ("readonly_dry_execution_rehearsal_suite", "scripts.run_testnet_readonly_dry_execution_rehearsal_suite"),
    ]
    for suite_id, suite_mod_path in suites:
        try:
            mod = importlib.import_module(suite_mod_path)
            rc = mod.main()
            results.append({"step_id": suite_id, "status": "PASS" if rc == 0 else "FAIL", "return_code": rc})
        except Exception as exc:
            results.append({"step_id": suite_id, "status": "FAIL", "error": str(exc)})
    return results


def main() -> int:
    start = time.time()
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    REPORT_DIR.mkdir(parents=True, exist_ok=True)

    print("=== T305001-T320000 Testnet Read-Only Final Governance Freeze Suite ===")
    print()

    print("--- Running previous milestone suites ---")
    prev_results = run_previous_suites()
    for r in prev_results:
        print(f"  {r['step_id']}: {r['status']}")
    print()

    print("--- Running final governance freeze steps ---")
    step_results = []
    for step_id, mod_path in STEPS:
        result = run_step(step_id, mod_path)
        step_results.append(result)
        print(f"  {step_id}: {result['status']}")
    print()

    all_results = prev_results + step_results
    passed = sum(1 for r in all_results if r["status"] == "PASS")
    failed = sum(1 for r in all_results if r["status"] == "FAIL")

    manifest = {
        "milestone": "T305001-T320000",
        "title": "Testnet Read-Only Final Governance Freeze",
        "total_steps": len(all_results),
        "passed": passed,
        "failed": failed,
        "elapsed_seconds": round(time.time() - start, 2),
        "steps": all_results,
        "submit_allowed": False,
        "testnet_submit_allowed": False,
        "real_submit_allowed": False,
    }
    (OUT_DIR / "final_governance_freeze_suite_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")

    lines = [
        "# Testnet Read-Only Final Governance Freeze Suite", "",
        f"**Total: {len(all_results)} | Passed: {passed} | Failed: {failed}**", "",
        "| Step | Status |", "|------|--------|",
    ]
    for r in all_results:
        lines.append(f"| {r['step_id']} | {r['status']} |")
    lines.extend(["", "## Conclusion", "",
        "TESTNET_READONLY_FINAL_GOVERNANCE_FREEZE_SUITE_PASS",
        "READONLY_FINAL_GOVERNANCE_FREEZE_READY",
        "READONLY_OPERATOR_HANDOFF_PACKET_READY",
        "NO_SUBMIT_RELEASE_ARCHIVE_READY",
        "READONLY_FINAL_GOVERNANCE_NO_NETWORK_NO_SUBMIT_SAFETY_PASS",
        "REAL_NETWORK_NOT_ALLOWED",
        "REAL_TRADING_NOT_ALLOWED",
        "TESTNET_SUBMIT_NOT_ALLOWED",
        ""])
    (REPORT_DIR / "testnet_readonly_final_governance_freeze_suite_report.md").write_text("\n".join(lines), encoding="utf-8")

    handoff_lines = ["# Final Read-Only Final Governance Freeze Handoff", "",
        "## Status", "",
        "TESTNET_READONLY_FINAL_GOVERNANCE_FREEZE_SUITE_PASS",
        "TESTNET_SUBMIT_NOT_ALLOWED",
        "REAL_TRADING_NOT_ALLOWED",
        "", "## Artifacts", "",
        "- Governance freeze: data/runtime/testnet_readonly_final_governance_freeze/final_governance_freeze.json",
        "- Operator handoff: data/runtime/testnet_readonly_final_governance_freeze/operator_handoff_packet.json",
        "- Release archive: data/runtime/testnet_readonly_final_governance_freeze/no_submit_release_archive.json",
        "- Safety regression: data/runtime/testnet_readonly_final_governance_freeze/final_governance_safety_regression.json",
        "- Suite manifest: data/runtime/testnet_readonly_final_governance_freeze/final_governance_freeze_suite_manifest.json",
        ""]
    (REPORT_DIR / "final_readonly_final_governance_freeze_handoff.md").write_text("\n".join(handoff_lines), encoding="utf-8")

    elapsed = round(time.time() - start, 2)
    print(f"=== Result: {passed}/{len(all_results)} passed in {elapsed}s ===")
    print("=== TESTNET_SUBMIT_NOT_ALLOWED ===")
    return 0 if failed == 0 else 1


if __name__ == "__main__":
    raise SystemExit(main())
