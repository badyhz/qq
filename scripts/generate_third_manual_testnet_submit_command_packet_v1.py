...
def generate_command_packet(eligibility: Optional[Dict[str, Any]], candidate: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if not (eligibility and eligibility.get("verdict") == "PASS" and eligibility.get("eligible_for_third_manual_submit") is True):
    ...
    dry_run_command = (
        ...
        "--env testnet --output <third_submit_result.json> --json"
    )
    submit_template = (
        "python3 scripts/run_testnet_submit_execution_wrapper_v1.py "
        "--inputs <command_packet.json> <token_gate.json> <payload.json> <invariants.json> <phase_report.json> "
        "--allow-testnet-submit --confirm-token <TOKEN> --env testnet "
        "--output <third_submit_result.json> --json"
    )
    ...
    return {
        ...
        "required_confirmation_token_source": "THIRD_MANUAL_CONFIRMATION_GATE",
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="Generate third manual testnet submit command packet")
