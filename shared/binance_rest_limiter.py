"""Cross-process Binance public REST guard with persistent IP-ban state.
...
def _default_state_path() -> Path:
    ...
    production = Path("/var/lib/quant-shared")
    if production.is_dir() and os.access(production, os.W_OK):
        return production / "binance_public_rate_limit.json"
...
class BinanceRestLimiter:
    ...
    def _empty_state() -> dict[str, Any]:
        return {
            ...
            "error_1003_count": 0,
            "server_time_count": 0,
            "maximum_observed_1m_weight": 0,
            "maximum_exchange_used_weight_1m": 0,
            "window": [],
        }
    ...
    def snapshot(self) -> dict[str, Any]:
        with self._locked_state() as state:
            ...
            result["current_1m_weight"] = sum(
                int(_number(row[1]))
                for row in list(state.get("window") or [])
                if isinstance(row, list) and len(row) == 2 and _number(row[0]) > now - 60.0
            )
            return result

    def acquire(self, *, weight: int, health_probe: bool = False, path: str = "") -> None:
        now = float(self.clock())
        requested_weight = max(1, int(weight))
        with self._locked_state() as state:
            ban_until = _number(state.get("ban_until"))
            if now < ban_until:
                state["skip_count"] = int(_number(state.get("skip_count"))) + 1
                raise BinanceRestBlocked("BINANCE_IP_BANNED", retry_at=ban_until)

            healthy = bool(state.get("public_read_healthy", True))
            recovery_probe_required = ban_until > 0 and now >= ban_until and not healthy
            ...
            next_request_at = _number(state.get("next_request_at"))
            if next_request_at > now:
                raise BinanceRestBlocked("BINANCE_REQUEST_PACING", retry_at=next_request_at)
            ...
            state["next_request_at"] = now + (60.0 * requested_weight / self.safe_weight_budget)
            state["total_requests"] = int(_number(state.get("total_requests"))) + 1
            state["total_weight"] = int(_number(state.get("total_weight"))) + requested_weight
            state["maximum_observed_1m_weight"] = max(
                int(_number(state.get("maximum_observed_1m_weight"))), used + requested_weight
            )
            if path in {"/api/v3/time", "/fapi/v1/time"}:
                state["server_time_count"] = int(_number(state.get("server_time_count"))) + 1

    def acquire_with_pacing(
        self,
        *,
        weight: int,
        ...
        """Wait only for local capacity; bans and recovery gates fail closed."""
        while True:
            try:
                self.acquire(weight=weight, health_probe=health_probe, path=path)
                return
            except BinanceRestBlocked as exc:
                if exc.reason not in {
                    "BINANCE_REQUEST_IN_FLIGHT",
                    "BINANCE_REQUEST_PACING",
                    "BINANCE_LOCAL_WEIGHT_BUDGET_EXHAUSTED",
                }:
                    raise
                now = float(self.clock())
    ...
    def observe(
        ...
        *,
        ...
        with self._locked_state() as state:
            ...
            state["maximum_exchange_used_weight_1m"] = max(
                int(_number(state.get("maximum_exchange_used_weight_1m"))), used_1m
            )
            if status == 429:
                state["status_429_count"] = int(_number(state.get("status_429_count"))) + 1
            if status == 418:
                state["status_418_count"] = int(_number(state.get("status_418_count"))) + 1
            if code == "-1003":
                state["error_1003_count"] = int(_number(state.get("error_1003_count"))) + 1
...
def run_binance_rest_call(
    ...
    *,
    ...
    wait_for_local_capacity: bool = True,
    sleeper: Callable[[float], None] = time.sleep,
) -> T:
    ...
    request_weight = weight or estimate_request_weight(url)
    if wait_for_local_capacity:
        active.acquire_with_pacing(
        ...
        active.acquire(weight=request_weight, health_probe=health_probe, path=parsed.path)
    try:
        response = call()
